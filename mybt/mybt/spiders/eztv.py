...
class EZTV(Spider):
	...
	def __init__(self, search, cat = "tv"):
	...
	def secondParse(self, response):
		...
		item['seeder'] = response.xpath('//td[@style="padding-left: 5px;"]/span[1]/text()').extract()[0]
		item['leecher'] = response.xpath('//td[@style="padding-left: 5px;"]/span[2]/text()').extract()[0]
		item['site'] = 'EZTV'
		if int(item['seeder']):
			yield item
